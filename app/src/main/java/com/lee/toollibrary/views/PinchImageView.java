...
public class PinchImageView extends ImageView {
    ...
    public float mImgeRight;
    public float mImgeLeft;
    public float mImgeViewWidth;
        ...
        if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL) {
            ...
            if (!(mScaleAnimator != null && mScaleAnimator.isRunning())) {
                ...
                if (mPinchMode == PINCH_MODE_SCROLL) {
                    ...
                    float offx = event.getX() - mDownX;
                    if (offx < 0 && mImgeRight == mImgeViewWidth) {
                        getParent().requestDisallowInterceptTouchEvent(false);
                        return false;
                    }
                    if (offx > 0 && mImgeLeft == 0) {
                        getParent().requestDisallowInterceptTouchEvent(false);
                        return false;
                    }
                    //记录新的移动点
    ...
    private boolean scrollBy(float xDiff, float yDiff) {
        ...
        mImgeRight = bound.right;
        mImgeLeft = bound.left;
        mImgeViewWidth = displayWidth;
    ...
    public void currentScale() {
        ...
        float innerScale = MathUtils.getMatrixScale(innerMatrix)[0];
        float outerScale = MathUtils.getMatrixScale(mOuterMatrix)[0];
        float currenScale = innerScale * outerScale;
        mCurrentScale = currenScale;
        if (mOldScale <= 0) {
            mOldScale = currenScale;
        }
    }
}