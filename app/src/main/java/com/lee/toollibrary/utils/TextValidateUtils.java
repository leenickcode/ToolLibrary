...
public class TextValidateUtils {
	 ...
	 * @return
	 */
	public static boolean isNickname2(String nickname) {
	...
	public static  boolean isQQ(String qq){
		...
		Matcher matcher = mPattern.matcher(qq);
		return matcher.find();
	}
	//验证微信是否合法
	public static  boolean isWEIXING(String wexin){
		mPattern = Pattern.compile(WEIXIN);
		Matcher matcher = mPattern.matcher(wexin);
		return matcher.find();
	}
	//验证身份证是否合法
	public static  boolean isIDcard(String qq){
		mPattern = Pattern.compile(ID_CARD);
		Matcher matcher = mPattern.matcher(qq);
		return matcher.find();
	}
	//志愿者验证用户名是否合法
	public static  boolean isVolunteerName(String name){
		mPattern = Pattern.compile(REG_NICKNAME);
